...
(function() {
    ...
    // ==========================================
    // 2. 卡密验证 (LicenseManager) —— 对接后端
    // ==========================================
    const LicenseManager = {
        // ⭐ 填你的后端地址（本地测试 http://localhost:3000/verify，
        //    部署后改成 http://你的服务器IP:3000/verify 或 https://域名/verify）
        VERIFY_URL: "https://license-server-ned-production.up.railway.app/verify",

        // 卡密格式（与后端保持一致）
        KEY_PATTERN: /^YCY-[A-Z0-9]{4}-[A-Z0-9]{4}-[A-Z0-9]{4}$/i,

        // 签名密钥 —— 随便改，越乱越好；别告诉别人
        SIGN_SALT: "YangYang-DeYi-2025-Sig-n0neSh@ll#User2026",
        ...
        getDeviceId: () => {
            ...
            ctx.fillStyle = "#069";
            ctx.fillText("ycy-fp-core-" + (navigator.hardwareConcurrency || 4), 2, 2);
            ...
            const s = info + LicenseManager.SIGN_SALT;
            let h = 2166136261;
            for (let i = 0; i < s.length; i++) {
                h ^= s.charCodeAt(i);
                h = Math.imul(h, 16777619);
            }
            h = (h >>> 0).toString(36).toUpperCase();
            return "YCY-" + h.substring(0, 4) + "-" + h.substring(4, 8) + "-" + h.substring(8, 12);
        },

        // -----------------------------
        // 2. 签名校验 —— 防用户在油猴存储里手动改值
        //    存储结构: { key, deviceId, activatedAt, sig }
        //    只要用户改了任何字段（比如把 key 改成别的值），sig 就对不上，脚本当没激活处理
        // -----------------------------
        makeSignature: (data) => {
            const raw = (data.key || "") + "|" +
                (data.deviceId || "") + "|" +
                (data.activatedAt || 0) + "|" +
                LicenseManager.SIGN_SALT;
            let h = 0x811c9dc5;
        ...
        readCachedLicense: () => {
            const raw = GM_getValue("ycy_license");
            if (!raw || typeof raw !== "object") return null;
            if (!raw.key || !raw.deviceId || !raw.sig || !raw.activatedAt) return null;
            const expectedSig = LicenseManager.makeSignature(raw);
            if (raw.sig !== expectedSig) return null;           // 用户改过 → 无效
            if (raw.deviceId !== LicenseManager.getDeviceId()) return null; // 设备对不上
            return raw;
        },

        saveLicenseCache: (key, deviceId) => {
            const data = { key: key, deviceId: deviceId, activatedAt: Date.now() };
            data.sig = LicenseManager.makeSignature(data);
            GM_setValue("ycy_license", data);
        },

        isAlreadyBoundMsg: (msg) => !!msg && (msg.includes("已在另一台") || msg.includes("达到上限") || msg.includes("already") || msg.includes("bound")),

        // -----------------------------
        // 3. 卡密输入弹窗
        // -----------------------------
        showInputDialog: (errorMsg, onSubmit, onCancel) => {
            const old = document.getElementById("ycyLicenseDialog");
            if (old) old.remove();

            const mask = document.createElement("div");
            mask.id = "ycyLicenseDialog";
            ...
            const html = "" +
                '<div style="width:min(92vw,430px);background:linear-gradient(160deg,#1e3a2d,#12281c);border:1px solid rgba(120,180,140,0.4);border-radius:18px;padding:26px 24px 22px;box-shadow:0 25px 70px rgba(0,0,0,0.6);color:#fff;">' +
                '<div style="font-size:21px;font-weight:700;letter-spacing:2px;text-align:center;margin-bottom:6px;">🐑 羊羊得意 · 卡密激活</div>' +
                '<div style="font-size:12px;color:rgba(255,255,255,0.6);text-align:center;margin-bottom:20px;">首次启动需要输入卡密，激活后在此设备永久可用</div>' +
                '<label style="font-size:13px;color:rgba(255,255,255,0.8);display:block;margin-bottom:8px;">卡密</label>' +
                '<input id="ycyLicenseInput" type="text" placeholder="YCY-XXXX-XXXX-XXXX" ' +
                'style="width:100%;box-sizing:border-box;padding:11px 14px;border-radius:10px;border:1px solid rgba(120,180,140,0.4);background:rgba(255,255,255,0.07);color:#fff;font-size:15px;letter-spacing:1px;outline:none;" autocomplete="off" spellcheck="false">' +
                '<div id="ycyLicenseMsg" style="min-height:22px;margin-top:10px;font-size:13px;color:#ff9b9b;"></div>' +
                '<div style="display:flex;gap:10px;margin-top:6px;">' +
                '<button id="ycyLicenseCancel" style="flex:1;padding:10px;border-radius:10px;border:1px solid rgba(255,255,255,0.25);background:transparent;color:#fff;font-size:14px;cursor:pointer;">取消</button>' +
                '<button id="ycyLicenseSubmit" style="flex:2;padding:10px;border-radius:10px;border:none;background:linear-gradient(135deg,#5a9b70,#3f7550);color:#fff;font-size:14px;font-weight:600;cursor:pointer;">激 活</button>' +
                '</div>' +
                '<div style="margin-top:16px;padding:11px 13px;border-radius:10px;background:rgba(0,0,0,0.3);font-size:11px;color:rgba(255,255,255,0.55);line-height:1.7;">' +
                '本设备: <span id="ycyDeviceId" style="color:rgba(255,255,255,0.75);font-family:Consolas,monospace;">—</span>' +
                '<br><span style="color:rgba(255,255,255,0.45);">一张卡密可激活手机/平板/电脑各 1 台，同类型设备无法重复激活</span>' +
                '</div></div>';
            mask.innerHTML = html;
            document.body.appendChild(mask);

            const deviceId = LicenseManager.getDeviceId();
            document.getElementById("ycyDeviceId").textContent = deviceId;

            const input = document.getElementById("ycyLicenseInput");
            const msg = document.getElementById("ycyLicenseMsg");
            if (errorMsg) msg.textContent = errorMsg;
            setTimeout(() => input.focus(), 80);
            input.addEventListener("keydown", e => {
                if (e.key === "Enter") document.getElementById("ycyLicenseSubmit").click();
                if (e.key === "Escape") document.getElementById("ycyLicenseCancel").click();
            });
            document.getElementById("ycyLicenseSubmit").onclick = () => {
                const key = input.value.trim().toUpperCase();
                if (!key) { msg.textContent = "请输入卡密"; return; }
                if (!LicenseManager.KEY_PATTERN.test(key)) { msg.textContent = "卡密格式不正确"; return; }
                msg.style.color = "#ffd37a";
                msg.textContent = "正在向服务器验证...";
                onSubmit(key, deviceId, (ok, err) => {
                    if (ok) mask.remove();
                    else { msg.style.color = "#ff9b9b"; msg.textContent = err || "验证失败"; }
                });
            };
            document.getElementById("ycyLicenseCancel").onclick = () => {
                mask.remove();
        ...
        showTip: (text, color) => {
            ...
            tip.style.cssText = "position:fixed;top:14px;left:50%;transform:translateX(-50%);padding:11px 22px;background:" + (color || "rgba(130,40,40,0.9)") + ";color:#fff;border-radius:24px;font-size:14px;z-index:2147483647;font-family:'Microsoft YaHei',Arial,sans-serif;box-shadow:0 6px 20px rgba(0,0,0,0.4);";
            tip.textContent = text;
            document.body.appendChild(tip);
            setTimeout(() => { if (tip.parentNode) tip.remove(); }, 4500);
        },

        // -----------------------------
        // 5. 网络验证
        // -----------------------------
        verifyKey: (key, deviceId, cb, activate) => {
            const url = LicenseManager.VERIFY_URL;
            if (!url) { cb(false, "脚本未配置验证服务器"); return; }
            try {
                GM_xmlhttpRequest({
                    method: "POST",
                    url: url,
                    headers: { "Content-Type": "application/json" },
                    data: JSON.stringify({ key: key, deviceId: deviceId, ua: navigator.userAgent, activate: !!activate }),
                    onload: (r) => {
                        try {
                            const data = JSON.parse(r.responseText);
                            if (data && data.ok) cb(true, data.msg || "验证通过");
                            else cb(false, (data && data.msg) || "卡密无效或已被其他设备绑定");
                        } catch (e) {
                            cb(false, "服务器返回异常");
                        }
                    },
                    onerror: () => cb(false, "网络异常，无法连接验证服务器"),
                    ontimeout: () => cb(false, "连接服务器超时")
                });
            } catch (e) {
                cb(false, "请求失败：" + e.message);
            }
        },
        ...
        // 6. 主流程（核心逻辑）
        //    规则：
        //    - 第一次启动 → 弹卡密输入框 → 验证成功 → 写入本地缓存（带签名）→ 以后不弹窗
        //    - 以后每次启动 → 读取本地缓存 → 校验签名和设备 ID → 通过就直接放行
        //    - 如果用户在油猴里手动改了缓存 → 签名对不上 → 重新弹窗
        //    - 后端按『设备类型』限制：手机/平板/电脑 各 1 台，同类型无法重复激活
        // -----------------------------
        requireLicense: (onPassed) => {
            const deviceId = LicenseManager.getDeviceId();
            const cached = LicenseManager.readCachedLicense();

            if (cached) {
                LicenseManager.verifyKey(cached.key, deviceId, (ok, msg) => {
                    if (ok) {
                        LicenseManager.saveLicenseCache(cached.key, deviceId);
                        onPassed();
                        return;
                    }
                    GM_setValue("ycy_license", "");
                    if (msg && msg.includes("已被禁用")) {
                        LicenseManager.showTip("🐑 卡密已被禁用，请输入新卡密");
                    } else {
                        LicenseManager.showTip("🐑 卡密已失效，请重新输入");
                    }
                    LicenseManager.showInputDialog(null, (k, d, cb) => LicenseManager.verifyKey(k, d, (ok2, err2) => {
                        if (ok2) { LicenseManager.saveLicenseCache(k, d); cb(true); setTimeout(onPassed, 150); }
                        else cb(false, err2);
                    }, true), () => LicenseManager.showTip("🐑 未激活卡密，脚本已停止运行"));
                }, false);
                return;
            }

            LicenseManager.showInputDialog(null, (k, d, cb) => LicenseManager.verifyKey(k, d, (ok, err) => {
                if (ok) {
                    LicenseManager.saveLicenseCache(k, d);
                    cb(true);
                    setTimeout(onPassed, 150);
                } else {
                    cb(false, err);
                }
            }, true), () => LicenseManager.showTip("🐑 未激活卡密，脚本已停止运行"));
    ...
    const AvgEngineHandler = {
        ...
        hookGlobalFunctions: () => {
            ...
            unsafeWindow.JSON.parse = function(text) {
                ...
                try { if (data.jsList) data.jsList.pop(); } catch {}
                ...
                try { if (data.commands?.length > 0) data.commands = data.commands.filter(cmd => !cmd.includes("createLock")); } catch {}
                    ...
                    setTimeout(async () => {
                        ...
                        if (text === null) {
                            try {
                                ...
                                text = text.replace("_0x289b9c(_0x303a27['currentGameId'])['then'](function(_0xd6c61c){", "_0x289b9c(_0x303a27['currentGameId'])['then'](function(_0xd6c61c){_0xd6c61c.data.cts={};");
                                text = text.replace("_0x303a27={", "_0x303a27=window.glb={");
                                text = text.replace("function(_0x195be5,_0x5950f1){", "function(_0x195be5,_0x5950f1){window.ymlfvar=_0x5cda97;if(window.$jksj){window.$jkddsj[`ac.var['${_0x5950f1}']`]=true};");
                                text = text.replace("function(_0x3adc13,_0x54acba){", "function(_0x3adc13,_0x54acba){window.ymlfarr=_0x2a6064;if(window.$jksj){window.$jkddsj[`ac.arr['${_0x54acba}']`]=true};");
                                text = text.replace("function(_0xaa21ec,_0x18c15e){", "function(_0xaa21ec,_0x18c15e){window.ymlfcVar=_0x2efdbd;if(window.$jksj){window.$jkddsj[`ac.cVar['${_0x18c15e}']`]=true};");
                                text = text.replace("function(_0x45bc30,_0x5f2be9){", "function(_0x45bc30,_0x5f2be9){window.ymlfcArr=_0x2c44a4;if(window.$jksj){window.$jkddsj[`ac.cArr['${_0x5f2be9}']`]=true};");
                            } catch {}
                        }
                    ...
                    const modified = lines.map(line => {
                        ...
                        if (trimmed.startsWith("ac.")) {
                            ...
                            if (eqInQuote !== -1) {
                                ...
                                const after = trimmed.substring(eqInQuote + 1).replace(/[^;]+/, newVal);
                                return before + after;
                            }
                        }
                        return line;
    ...
    setTimeout(() => {
        try {
            LicenseManager.requireLicense(mainFunction);
        } catch (e) {}
    }, 100);
})();
